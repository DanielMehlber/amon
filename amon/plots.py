...
def intensity_figure(event: dict, width: int = 520, height: int = 240):
    ...
    fig.line(xs, ys, line_width=2, color="#1f77b4", legend_label="intensity")
    threshold = Span(location=event["threshold"], dimension="width",
                     line_color="#d62728", line_dash="dashed", line_width=2)
    fig.add_layout(threshold)
    fig.line([], [], color="#d62728", line_dash="dashed", legend_label="threshold")
